...
import Verbose.Tactics.CC.MkProof
...
namespace Verbose.Tactics.CC.CCM
...
def dbgTraceACEq (header : String) (lhs rhs : ACApps) : CCM Unit := do
  let ccs ← get
  trace[Verbose.Debug.Meta.Tactic.cc.ac]
...
def dbgTraceACState : CCM Unit := do
  let ccs ← get
  trace[Verbose.Debug.Meta.Tactic.cc.ac] group ("state: " ++ nest 6 ccs.ppAC)
...
def composeAC (lhs rhs : ACApps) (H : DelayedExpr) : CCM Unit := do
  ...
  for Rlhs in occs do
    ...
    if lhs.isSubset Rrhs then
      ...
      let ccs ← get
      trace[Verbose.Debug.Meta.Tactic.cc.ac] group <|
        let oldRw :=
...
def processAC : CCM Unit := do
  repeat
    ...
    if lhs == rhs then
      trace[Verbose.Debug.Meta.Tactic.cc.ac] "trivial"
      continue
...
def removeParents (e : Expr) (parentsToPropagate : Array Expr := #[]) : CCM (Array Expr) := do
  ...
  for pocc in ps do
    let p := pocc.expr
    trace[Verbose.Debug.Meta.Tactic.cc] "remove parent: {p}"
...
def reinsertParents (e : Expr) : CCM Unit := do
  ...
  for p in ps do
    trace[Verbose.Debug.Meta.Tactic.cc] "reinsert parent: {p.expr}"
...
where
  ...
  go (e₁ e₂: Expr) (n₁ n₂ r₁ r₂ : Entry) (flipped : Bool) (H : EntryExpr) (heqProof : Bool) :
    ...
    let e₁Root := n₁.root
    let e₂Root := n₂.root
    trace[Verbose.Debug.Meta.Tactic.cc] "merging\n{e₁} ==> {e₁Root}\nwith\n{e₂Root} <== {e₂}"
    ...
    let ccs ← get
    trace[Verbose.Meta.Tactic.cc.merge] "{e₁Root} = {e₂Root}"
    trace[Verbose.Debug.Meta.Tactic.cc] "merged: {e₁Root} = {e₂Root}\n{ccs.ppEqcs}"
    trace[Verbose.Debug.Meta.Tactic.cc.parentOccs] ccs.ppParentOccs
