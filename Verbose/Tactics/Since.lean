...
/-- Try to close the current goal using `grind only` with the given hypotheses
(clearing all other hypotheses first) and report whether it succeeded.
The tactic state is preserved in case of failure.
-/
def tryGrindStrict (goal : MVarId) (hyps : Array FVarId) : TacticM Bool := do
  let goals ← getGoals
  let state ← saveState
  goal.withContext do
  setGoals [goal]
  try
    ...
    let fvarsToClear := allFVars.filter fun fvar => !hyps.contains fvar
...
/-- Try to close the current goal using `grind only` with the given hypotheses and report
whether it succeeded. If the goal is an inequality, try to prove the corresponding equality
using `grind only` with the given hypotheses and report whether it succeeded.
The tactic state is preserved in case of failure.
-/
def tryGrindStrict! (goal : MVarId) (hyps : Array FVarId) : TacticM Bool := do
  if ← tryGrindStrict goal hyps then
    trace[Verbose] "grind worked"
    return true
  else
    trace[Verbose] "grind failed"
    return false
...
def tryAll_core (goal : MVarId) (factsT : Array Term) (factsFVar : Array FVarId) :
  ...
  if ← factsFVar.anyM isEqEqv then
    if ← withTraceNode `Verbose (fun e ↦ do return s!"{emo e} Will now try grind") do
      tryGrindStrict! goal factsFVar then return
...
* try `grind` using the given facts (only) if those facts include an equality or equivalence
