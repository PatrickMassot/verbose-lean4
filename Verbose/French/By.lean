...
elab "Par " e:maybeApplied " on obtient " colGt news:newStuff : tactic => do
obtainTac (← maybeAppliedToTerm e) (newStuffToArray news) <|> anonymousLemmaTac (← maybeAppliedToTerm e) (newStuffToArray news)

elab "Par " e:maybeApplied " on choisit " colGt news:newStuff : tactic => do
chooseTac (← maybeAppliedToTerm e) (newStuffToArray news)

elab "Par " e:maybeApplied " il suffit de montrer " "que "? colGt arg:term : tactic => do
bySufficesTac (← maybeAppliedToTerm e) #[arg]

elab "Par " e:maybeApplied " il suffit de montrer " "que "? colGt "["args:term,*"]" : tactic => do
